...
describe('fetcher', () => {
  ...
  it('throws friendly message for 502', async () => {
    ...
    await expect(fetcher('/api/test')).rejects.toThrow('サイトに接続できませんでした')
  })

  it('throws friendly message for 503', async () => {
    globalThis.fetch = mockFetch(503, {})
    await expect(fetcher('/api/test')).rejects.toThrow('サイトが一時的に利用できません')
  })

  it('throws friendly message for 504', async () => {
    globalThis.fetch = mockFetch(504, {})
    await expect(fetcher('/api/test')).rejects.toThrow('タイムアウトしました')
  })
  ...
  it('falls back to status text when body is not JSON', async () => {
    ...
    await expect(fetcher('/api/test')).rejects.toThrow('500 Internal Server Error')
  })
})
